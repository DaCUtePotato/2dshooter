...
# Function to draw tiles on the screen
def draw_tiles():
    for y in range(0, height, tile_size):
        for x in range(0, width, tile_size):
            screen.blit(scaled_tile_image, (x, y))

# Function to handle shooting the base gun
...
# Function to draw the kill counter
def draw_kill_counter(kills):
    kills_text = menu_font.render(f"Kills: {kills}", True, RED)
    text_width, text_height = menu_font.size(f"Kills: {kills}")
...
def draw_exp_bar():
    ...
    exp_text = menu_font.render(f"EXP: {exp}/{max_exp}", True, WHITE)
    text_width, text_height = menu_font.size(f"EXP: {exp}/{max_exp}")
...
def draw_hp_bar():
    ...
    hp_text = menu_font.render(f"{player_hp}/100 HP", True, WHITE)
